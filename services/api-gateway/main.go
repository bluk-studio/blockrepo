...
func makeOriginFunc() func(origin string) bool {
...
func main() {
	...
	confFile := getEnv("GRPC_GW_CONF", "./config.yml")
	...
	for _, cli := range appConfig.Endpoints {
		...
		if err != nil {
			log.Println("Dial panic")
			panic(err)
		}

		err, methods, fds := DiscoverServices(conn)
		if err != nil {
			log.Println("Discover service panic")
			panic(err)
		}
		...
		for _, m := range methods {
			...
			if strings.HasPrefix(m, "/grpc.reflection.") {
				// ignore reflection.
				log.Println("ignore reflection method")
				continue
			}
	...
	director := func(ctx context.Context, fullMethodName string) (context.Context,
		*grpc.ClientConn, error) {
		...
		return ctx, nil, grpc.Errorf(codes.Unimplemented, "not found")
	}
	...
	RegisterCustomReflection(server, fdsCollected)
	...
	serverOptions := []grpcweb.Option{
		grpcweb.WithCorsForRegisteredEndpointsOnly(false),
		grpcweb.WithOriginFunc(makeOriginFunc()),
	}

	wrappedServer := grpcweb.WrapServer(server, serverOptions...)
	handler := func(resp http.ResponseWriter, req *http.Request) {
		if wrappedServer.IsGrpcWebRequest(req) {
			log.Println("serve grpc-web request")
			wrappedServer.HandleGrpcWebRequest(resp, req)
		} else {
	...
	log.Println("http server listening at: ", httpServer.Addr)
